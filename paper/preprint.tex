...
\begin{abstract}
Modern language-model evaluation is a coupled measurement procedure rather than a transparent scalar observation: prompt distributions, benchmark axes, judge protocols, rubrics, decoding rules and aggregation weights can all enter the final score. This manuscript isolates a finite mathematical obstruction relevant to residual audits of such procedures. On a finite positive weighted two-way table $X=Q\times B$, with weighted Hilbert space $L^2(w)$, let $C$ be the constant subspace, $A$ the centred row-main-effect subspace and $B_0$ the centred column-main-effect subspace. We prove that product weights are equivalent to $A\perp B_0$, and exactly equivalent to order-independence of the two sequential stripping maps $(I-P_{B_0})(I-P_A)(I-P_C)$ and $(I-P_A)(I-P_{B_0})(I-P_C)$. Under non-product weights, there exists a pure main-effect witness whose true additive residual is zero while the wrong stripping order produces a non-zero residual-like output. An exact $2\times2$ rational certificate is given, with squared weighted norm $61/177408$. The MaoField part of the manuscript is programmatic only: it proposes residual metric audits as future work for studying whether observed capability, safety, reasoning, hallucination, alignment and benchmark scores are invariant model properties or coordinate-dependent artifacts of a weighted evaluation field. No empirical MaoField result is reported.
\end{abstract}

\paragraph{Scope and claim boundary.}
The established mathematical contribution of this manuscript is the finite-dimensional theorem and its exact rational witness. The MaoField sections are a research programme and set of proposed future audit objects. They do not assert a new broad ANOVA theory, a new dependent-input decomposition theory, a new theory of noncommuting projections, or an empirical finding about language models. The duplicate-risk status is MEDIUM duplicate risk. Mode B MaoField empirical status remains \texttt{insufficient\_artifact}; accordingly, no MaoField empirical positive claim is made or authorized anywhere in this manuscript.

\section{Introduction: residuals inside black-box evaluation}

Large language model evaluation is increasingly mediated by complex measurement systems. Large benchmark suites aggregate heterogeneous tasks; holistic evaluations track multiple desiderata; open-ended generation is often judged by human raters or by other language models; and contamination, rephrasing and benchmark exposure can change what a score measures \cite{bigbench2022,helm2022,mtbench2023,geval2023,truthfulqa2021,selfcheckgpt2023,decodingtrust2023,llmevalsurvey2023,contamination2024,rethinkingcontamination2023}. The object being audited is therefore not only a model. It is a model together with a measurement field: prompts, tasks, judges, rubrics, sampling rules, weights and aggregation charts.
...
The theorem is deliberately small. It does not explain deployed LLM systems. Its role is to provide a minimal obstruction: if residuals can become procedural artifacts in a finite weighted table, larger black-box metric systems require explicit audits of weighting, nuisance-removal order, judge protocol and aggregation chart before residual-like signals are interpreted as intrinsic model properties.
...
\item It formulates a MaoField residual metric audit programme as future work: a metric residual should not be interpreted as an intrinsic model property until its invariance under weighting, nuisance-removal order, judge protocol and aggregation chart has been audited.
\end{enumerate}
...
\subsection{Metrics as fields rather than transparent scores}
...
\section{Future mathematical objects}

The theorem suggests several mathematical objects for future work.
\begin{enumerate}[leftmargin=*,itemsep=2pt]
...
Srivastava, A. et al. Beyond the Imitation Game: Quantifying and extrapolating the capabilities of language models. arXiv:2206.04615v3 (2022).

\bibitem{helm2022}
Liang, P. et al. Holistic Evaluation of Language Models. arXiv:2211.09110v2 (2022).
...
Il Idrissi, M. et al. Hoeffding decomposition of functions of random dependent variables. Journal of Multivariate Analysis 208, 105444 (2025). doi:10.1016/j.jmva.2025.105444.
...
B\"ottcher, A. and Spitkovsky, I. M. A gentle guide to the basics of two projections theory. Linear Algebra and its Applications 432, 1412--1459 (2010). doi:10.1016/j.laa.2009.11.002.
